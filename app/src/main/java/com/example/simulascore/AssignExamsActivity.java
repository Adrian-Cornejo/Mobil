...
public class AssignExamsActivity extends AppCompatActivity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_assign_exams);
            ...
            JsonObjectRequest request = new JsonObjectRequest(Request.Method.GET, url, null,
                        ...
                        public void onResponse(JSONObject response) {
                            try {
                                if (response.getString("status").equals("success")) {
                                    ...
                                    Toast.makeText(AssignExamsActivity.this, response.getString("message"), Toast.LENGTH_SHORT).show();
                                }
                            } catch (JSONException e) {
                                e.printStackTrace();
                                Toast.makeText(AssignExamsActivity.this, "Error parsing response", Toast.LENGTH_SHORT).show();
                            }
                        }
                    },
                    new Response.ErrorListener() {
                        ...
                        public void onErrorResponse(VolleyError error) {
                            ...
                            Toast.makeText(AssignExamsActivity.this, "Error fetching students", Toast.LENGTH_SHORT).show();
                        }
                    });
    ...
    private void assignExamToGroup() {
        ...
        params.put("tipoExamen", "resultados_examen_mejoredu");
        params.put("fechaLimite", date);
        params.put("horaLimite", time);

        Log.d("AssignExamsActivity", "Params: " + params.toString()); // Agregar registro para depurar

        JsonObjectRequest request = new JsonObjectRequest(Request.Method.POST, url, new JSONObject(params),
                new Response.Listener<JSONObject>() {
                    @Override
                    public void onResponse(JSONObject response) {
                        Log.d("AssignExamsActivity", "Response: " + response.toString()); // Registrar la respuesta

                        try {
                            String status = response.getString("status");
                            String message = response.getString("message");
                            if (status.equals("success")) {
                                Toast.makeText(AssignExamsActivity.this, message, Toast.LENGTH_SHORT).show();
                            } else {
                                Toast.makeText(AssignExamsActivity.this, "Error: " + message, Toast.LENGTH_SHORT).show();
                            }
                        } catch (JSONException e) {
                            Log.e("AssignExamsActivity", "JSON parsing error: " + e.getMessage());
                            Toast.makeText(AssignExamsActivity.this, "Error parsing response", Toast.LENGTH_SHORT).show();
                        }
                    }
                },
                    ...
                    public void onErrorResponse(VolleyError error) {
                        if (error.networkResponse != null) {
                            try {
                                ...
                                Log.e("AssignExamsActivity", "Raw response: " + responseBody); // Registro de la respuesta cruda

                                // Verifica si la respuesta es HTML
                                if (responseBody.startsWith("<br")) {
                                    Log.e("AssignExamsActivity", "Received HTML response instead of JSON");
                                    Toast.makeText(AssignExamsActivity.this, "Error del servidor: Respuesta no válida", Toast.LENGTH_SHORT).show();
                                } else {
                                    JSONObject data = new JSONObject(responseBody);
                                    String message = data.optString("message");
                                    Log.e("AssignExamsActivity", "Server error: " + message);
                                    Toast.makeText(AssignExamsActivity.this, "Error: " + message, Toast.LENGTH_SHORT).show();
                                }
                            } catch (Exception e) {
                                Log.e("AssignExamsActivity", "Error parsing server error response: " + e.getMessage());
                                e.printStackTrace();
                            }
                        } else {
                            Log.e("AssignExamsActivity", "Network error: " + error.getMessage());
                            Toast.makeText(AssignExamsActivity.this, "Error assigning exam to group", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
                    ...
                    public void onErrorResponse(VolleyError error) {
                        if (error.networkResponse != null) {
                            try {
                                ...
                                Log.e("AssignExamsActivity", "Error parsing server error response: " + e.getMessage());
                                Log.e("AssignExamsActivity", "Response content: " + error.networkResponse.data.toString()); // Agregar este registro
                                e.printStackTrace();
                            }
                        } else {
                            Log.e("AssignExamsActivity", "Network error: " + error.getMessage());
                            Toast.makeText(AssignExamsActivity.this, "Error assigning exam to students", Toast.LENGTH_SHORT).show();
                        }
                    }
                });
